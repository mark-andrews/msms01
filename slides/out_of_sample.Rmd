...
$$
\sum_{i=1}^n (y_i - \hat{y}_i)^2 + \lambda \sum_{k=0}^K w_k^2,
$$
where
$$
\hat{y}_i = \beta_0 + \sum_{k=1}^K w_k x_{ki},
$$
...
$$
\sum_{i=1}^n (y_i - \hat{y}_i)^2 + \lambda \sum_{k=0}^K |w_k|.
$$
...
$$
\sum_{i=1}^n (y_i - \hat{y}_i)^2 + \lambda \left( \alpha \sum_{k=0}^K |w_k| + (1-\alpha)\sum_{k=0}^K w^2_k \right).
$$
